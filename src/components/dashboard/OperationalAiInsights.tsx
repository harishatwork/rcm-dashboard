...
import type { OperationalAiInsight } from "@/lib/api/operational-dashboard";
import { toast } from "sonner";

export function OperationalAiInsights({ insights }: { insights: OperationalAiInsight[] }) {
  const [appliedActions, setAppliedActions] = useState<Record<string, boolean>>({});

  const handleAction = (item: OperationalAiInsight) => {
    setAppliedActions((prev) => ({ ...prev, [item.id]: true }));
    toast.success(`Operational plan initiated: "${item.category}"`, {
      description: `Target location: ${item.locationName ?? "All Facilities"}. Est cost impact: ${formatCurrency(item.estimatedImpact)}`,
    });
  };
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-display text-base font-bold tracking-tight flex items-center gap-2">
              AI Operational Efficiency & Patient Flow Insights
            </h3>
            <p className="text-xs text-muted-foreground">
              Automated observations on lobby arrival surges, wait times, provider schedule utilization, and no-shows
            </p>
          </div>
        </div>
