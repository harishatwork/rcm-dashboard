...
import type { AiNotificationRecommendation } from "@/lib/api/notifications";
import { toast } from "sonner";

export function AiNotificationRecommendations({
  recommendations,
}: {
  recommendations: AiNotificationRecommendation[];
}) {
  const [appliedActions, setAppliedActions] = useState<Record<string, boolean>>({});

  const handleAction = (item: AiNotificationRecommendation) => {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-display text-base font-bold tracking-tight flex items-center gap-2">
              AI Recommendations & Alert Triggers
            </h3>
            <p className="text-xs text-muted-foreground">
              Automated artificial intelligence recommendations addressing high denial rates, AR spikes, slow-paying payors, and bottlenecks
            </p>
          </div>
        </div>
