...
import type { ProviderAiInsight } from "@/lib/api/provider-performance-dashboard";
import { toast } from "sonner";

export function ProviderAiInsights({ insights }: { insights: ProviderAiInsight[] }) {
  const [appliedActions, setAppliedActions] = useState<Record<string, boolean>>({});

  const handleAction = (item: ProviderAiInsight) => {
    setAppliedActions((prev) => ({ ...prev, [item.id]: true }));
    toast.success(`Efficiency plan assigned: "${item.category}"`, {
      description: `Target provider: ${item.providerName ?? "Clinical Team"}. Est revenue impact: ${formatCurrency(item.estimatedImpact)}`,
    });
  };
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-display text-base font-bold tracking-tight flex items-center gap-2">
              AI Provider Performance & Efficiency Insights
            </h3>
            <p className="text-xs text-muted-foreground">
              Automated observations on provider revenue yield, wRVU trends, and denial exposure
            </p>
          </div>
        </div>
        <Badge variant="secondary" className="rounded-full px-3 py-1 text-xs font-semibold gap-1">
          <Zap className="h-3 w-3 text-amber-500 fill-amber-500" />
          3 Provider Action Plans
        </Badge>
      </div>
