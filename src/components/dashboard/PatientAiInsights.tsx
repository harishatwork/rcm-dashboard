...
import type { PatientAiInsight } from "@/lib/api/patient-analytics";
import { toast } from "sonner";

export function PatientAiInsights({ insights }: { insights: PatientAiInsight[] }) {
  const [appliedActions, setAppliedActions] = useState<Record<string, boolean>>({});

  const handleAction = (item: PatientAiInsight) => {
    setAppliedActions((prev) => ({ ...prev, [item.id]: true }));
    toast.success(`Action initiated: "${item.category}"`, {
      description: `Targeting growth & retention. Est revenue impact: ${formatCurrency(item.estimatedImpact)}`,
    });
  };
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-display text-base font-bold tracking-tight flex items-center gap-2">
              AI Patient Analytics & Retention Insights
            </h3>
            <p className="text-xs text-muted-foreground">
              Automated observations on patient acquisition, no-show patterns, and high-value retention
            </p>
          </div>
        </div>
