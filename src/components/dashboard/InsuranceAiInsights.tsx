...
import type { InsuranceAiInsight } from "@/lib/api/insurance-dashboard";
import { toast } from "sonner";

export function InsuranceAiInsights({ insights }: { insights: InsuranceAiInsight[] }) {
  const [appliedActions, setAppliedActions] = useState<Record<string, boolean>>({});

  const handleAction = (item: InsuranceAiInsight) => {
    setAppliedActions((prev) => ({ ...prev, [item.id]: true }));
    toast.success(`Strategy executed: "${item.category}"`, {
      description: `Target payor: ${item.payorName ?? "Multi-payor"}. Est impact: ${formatCurrency(item.estimatedImpact)}`,
    });
  };
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-display text-base font-bold tracking-tight flex items-center gap-2">
              AI Insurance Performance & Contract Insights
            </h3>
            <p className="text-xs text-muted-foreground">
              Automated observations on payor speed, collection efficiency, and AR exposure
            </p>
          </div>
        </div>
            ...
            >
              <div className="space-y-2">
                ...
                <div className="rounded-lg bg-muted/60 p-2.5 text-xs text-foreground font-medium border border-border/50">
                  <span className="font-semibold text-primary">Action Plan: </span>
                  {item.recommendation}
                </div>
              </div>
